<?php
    ...
    */
    ...
    class AuthItemChild extends \yii\db\ActiveRecord
    {
        ...
        {
            return 'tb_auth_item_child';
        }

        public static function getChild($parent = null, &$childArray)
        {
            $queryData1 = AuthItemChild::find()->where(['parent' => $parent])->andWhere('child not like :child1 and child not like :child2', [':child1' => 'order:%', ':child2' => 'frontend:%'])->asArray()->all();
            if ($queryData1) {
        ...
        {
            $queryData1 = AuthItemChild::find()->where(['child' => $child])->andWhere('parent not like :parent1 and parent not like :parent2', [':parent1' => 'order:%', ':parent2' => 'frontend:%'])->asArray()->all();
            if ($queryData1) {
                foreach ($queryData1 as $key => $value) {
