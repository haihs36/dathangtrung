<?php
    ...
    class TbOrders extends \yii\db\ActiveRecord
    {
        ...
        {
            return [
                [['identify', 'customerID'], 'required', 'message' => '{attribute} là bắt buộc'],
                [['weightDiscount', 'discountKg', 'cny'], 'customRequired', 'skipOnError' => false, 'skipOnEmpty' => false],

                [['isBox', 'requestPay', 'isCheck', 'isBad','isMobile', 'categoryId', 'customerID', 'orderStaff', 'provinID', 'businessID', 'staffsID', 'paymentID', 'totalQuantity', 'shipperID', 'orderNumber', 'active', 'status', 'shippingStatus', 'perCent', 'quantity'], 'integer'],
                ...
                [['orderDate', 'updatetime', 'setDate', 'shipDate', 'deliveryDate', 'finshDate', 'buyDate', 'shippingDate', 'vnDate', 'paymentDate', 'sourceName', 'discount'], 'safe'],
                [['identify', 'shipAddress', 'name', 'image', 'note_company', 'categoryName'], 'string'],
                [['businessID', 'orderStaff'], 'required', 'message' => '{attribute} là bắt buộc', 'on' => 'assign_order']
            ];
        }
        ...
        public function scenarios()
        {
            ...
            $arrSenarios = [
                'assign_order' => ['businessID', 'orderStaff'],
            ];
        ...
        public function attributeLabels()
        {
            return [
                ...
                'businessID'     => 'Nhân viên kinh doanh',
                'orderStaff'     => 'Nhân viên đặt hàng',
        ...
        public function getAction()
        {
            ...
            if ($this->status == 2) {
                switch ($user->role) {
                    case ADMIN:
                    case CLERK:
                        if ($this->requestPay == 1) {
                            $isBookOrder = '<a data-msg="Bạn có chắc chắn muốn chuyển trạng thái sang đã đặt hàng không?" class="booking btn btn-danger" href="' . Url::to(['orders/booking', 'id' => $this->primaryKey]) . '" data-toggle="tooltip" data-original-title="Đã đặt hàng"><i class="fa fa-shopping-cart" aria-hidden="true"></i> Đã đặt hàng</a>';
                        }
                        break;
                    case STAFFS: //NVDH

                        if ($this->requestPay != 1) {
                            $isBookOrder = '<a data-msg="Bạn có chắc chắn muốn gửi yêu cầu thanh toán không?"  class="payment-request btn btn-danger" href="' . Url::to(['orders/prequest', 'id' => $this->primaryKey]) . '" data-toggle="tooltip" data-original-title="Yêu cầu thanh toán"><i class="fa fa-shopping-cart" aria-hidden="true"></i> Yêu cầu thanh toán</a>';
                        } else {
                            $isBookOrder = '<span class=" btn btn-warning">Đã gửi yêu cầu</span>';
                        }
            ...
            return '<div class="dropdown actions">   
                    <span style="cursor: pointer"  data-toggle="dropdown" aria-expanded="true"><i class="glyphicon glyphicon-menu-hamburger"></i> </span>        
                     ' . $isBookOrder . '  
        ...
        public function actionOut()
        {
            ...
            $complain = '<a href="' . Url::to(['complain/create', 'id' => $this->primaryKey]) . '" data-toggle="tooltip"  data-original-title="Khiếu nại đơn hàng"  title="Khiếu nại đơn hàng"  class="tabledit-delete-button btn btn-danger waves-effect waves-light"><i class="fa fa-fw fa-frown-o"></i></a>';
            // }
            $message = '';
            ...
            $del = '';
            if ($this->status == 1) {
                $del = '<a href="' . Url::to(['orders/delete', 'id' => $this->primaryKey]) . '" title="Xóa đơn hàng" data-toggle="tooltip" onclick="return confirm(\'Bạn có chắc chắn muốn xóa đơn hàng này không ?\')" class="tabledit-delete-button btn btn-danger waves-effect waves-light"><i class="glyphicon glyphicon-trash" aria-hidden="true"></i> </a>';
            }
            ...
            return '<div class="btn-group btn-group-sm" role="group">
                        <a target="_blank" href="' . Url::toRoute(['orders/view', 'id' => $this->primaryKey]) . '" data-toggle="tooltip"  data-original-title="Chi tiết đơn hàng" title="Chi tiết đơn hàng" class="tabledit-edit-button btn btn-primary waves-effect waves-light"><i class="fa fa-fw fa-eye"></i></a>                       
                         ' . $complain . '
                         ' . $del . '
...
//            ->where(['a.status'=>$status,'a.isSelected'=> 1,'o.customerID'=>$customerID]);
        ...
        {
            $query = new \yii\db\Query;
            $query->select('d.id,o.orderID,d.qty_receive,d.note_receive,s.supplierID,s.sourceName,p.shopProductID, ,d.unitPrice,d.totalPrice,d.unitPriceVn,d.totalPriceVn,d.image,d.color,d.size,d.noteProduct,d.quantity,p.name as title,p.link,d.status,s.shopID,s.shopName')
                ->from('tb_orders o')
        ...
        //    api
        public static function getOrderDetailApi($orderID, $userID = 0, $statusProduct = false)
        {
            $query = new \yii\db\Query;
            $query->select('d.id,o.orderID,s.supplierID ,d.unitPrice,d.totalPrice,d.unitPriceVn,d.totalPriceVn,d.image,d.color,d.size,d.noteProduct,d.quantity,p.name as title,p.link,d.status,s.shopID,s.shopName')
                ->from('tb_orders o')
        ...
        public static function getOrderCount($isBook = null, $isCustomer = false)
        {
            ...
            if ($isBook || $user->role == WAREHOUSE) {
                ...
                $query->andFilterWhere(['active' => 1]);//don hang co canh bao
            }
            ...
            switch ($user->role) {
                case BUSINESS: //KD
                    ...
                    break;
                case STAFFS: //vn dat hang
                    $query->andFilterWhere(['orderStaff' => (int)$user->id]);
                    break;
                case COMPLAIN: //vn khieu nai
        ...
        /*public static function getTotalCoc($provider, $columnName)
        {
            ...
            public static function getTotalBottom($provider, $columnName)
        {
            $total = 0;
            foreach ($provider as $item) {
               $total += $item[$columnName];
