...
class TbService extends \yii\db\ActiveRecord
{
    ...
    {
        return [
            [['from', 'to', 'percent','provinID'], 'required','message'=>'{attribute} là bắt buộc'],
            [['percent'], 'number'],
            [['from','to'], 'isPrice'],
    ...
    public function isPrice( $attribute, $params ) {
        ...
        if(!is_numeric($this->from)){
            $this->addError('from', 'Giá cân nặng không hợp lệ');
            return false;
        }
        if(!is_numeric($this->to)){
            $this->addError('to', 'Giá cân nặng không hợp lệ');
            return false;
        }
        return true;
    }
    ...
    public function customRequired( $attribute, $params ) {
        ...
        elseif ($this->from >= $this->to) {
            $this->addError('from', 'Khoảng giá không hợp lệ.');
            return false;
        }
        return true;
    }
    ...
    public function getAction()
    {
        return '<div class="dropdown actions">
                    ...
                    <ul class="dropdown-menu dropdown-menu-right" role="menu" aria-labelledby="dropdownMenu13">
                         <li><a href="' . Url::to(['service/update', 'id' => $this->primaryKey]) . '"><i class="glyphicon glyphicon-pencil font-12"></i> Sửa</a></li>
                         <li>
                            <a href="' . Url::to(['service/delete', 'id' => $this->primaryKey]) . '" class="confirm-delete"><i class="glyphicon glyphicon-remove font-12"></i> Xóa</a>
                         </li>
                    </ul>
                </div>';
    }
