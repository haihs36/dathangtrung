<?php
    ...
    class AjaxController extends Controller
    {
        ...
        /**
         * @author cuonghh
         * @description ajax update order detail quantity buy
         */
        public function actionQuantityBuy()
        {
            $success = false;
            if (\Yii::$app->request->isAjax && \Yii::$app->request->isPost) {
                $detail_id = (int)\Yii::$app->request->post('pid');
                ...
                if ($orderDetail = TbOrdersDetail::findOne($detail_id)) {
                    if (!empty($note)) {
                        $orderDetail->note_buy = $note;
                    }
                    if (isset($qty) && intval($qty) >= 0) {
                        $orderDetail->qty_buy = $qty;
                    }
        ...
        public function actionUploadImage()
        {
            ...
            if (!empty($temp)) {
                ...
                if ($photo->image && $photo->validate(['image'])) {
                    ...
                    echo json_encode(['location' => Yii::$app->params['FileDomain'] . $photo]);
                }
            }
            ...
            if (\Yii::$app->request->isAjax && \Yii::$app->request->isPost) {
                ...
                if (!empty($params)) {
                    if ($consignModel = Consignment::findOne((int)$params['consignID'])) {
                        ...
                        $totalMoney = (float)$params['vitual_pay'];
                        if ($totalMoney <= 0) {
        ...
        public function actionAssignOrders()
        {
            ...
            if ($model->load(\Yii::$app->request->post())) {
                if (ActiveForm::validate($model)) {
                    ...
                    if (empty($reqData['TbOrders']['businessID']) && empty($reqData['TbOrders']['orderStaff'])) {
                        return ['mess' => '<div align="center">Lỗi! Bạn vui lòng chọn nhân viên kinh doanh, đặt hàng</div>'];
                    }
                    //create arr id
        ...
        public function actionAssignCustomers()
        {
            ...
            if ($model->load(\Yii::$app->request->post())) {
                if (ActiveForm::validate($model)) {
                    ...
                    if (empty($reqData['TbCustomers']['userID']) && empty($reqData['TbCustomers']['staffID'])) {
                        return ['mess' => '<div align="center">Lỗi! Bạn vui lòng chọn nhân viên kinh doanh, đặt hàng</div>'];
                    }
                    //create arr id
        ...
        /**
         * @author cuonghh
         * @function ajax table history shop
         */
        public function actionHistoryShop()
        {
            $supId = Yii::$app->request->get('supId');
            $orderId = Yii::$app->request->get('orderId');
            if (empty($supId)) {
                return ['mess' => '<div align="center">Lỗi! Không có id của shop</div>'];
            }
            $listHistory = (new Query())
                ->select([
                    'o.orderID', 'o.identify'
                ])
                ->from(TbOrders::tableName() . ' o')
                ->innerJoin(TbOrderSupplier::tableName() . ' s', 'o.orderID = s.orderID')
                ->where(['s.supplierID' => (int)$supId])
                ->andWhere(['<>', 'o.orderID', (int)$orderId])
                ->orderBy(['o.orderID' => SORT_DESC])
                ->limit(100)
                ->all();
            return $this->renderAjax('@app/views/ajax/_tbl_history_shop', [
        ...
        /**
         * @author cuonghh
         * @function ajax update order detail
         * @description update quantity, price
         */
        public function actionUpdateOrderDetail()
        {
            ...
            if ($reqData) {
                //validate request
                if (empty($reqData['arrId'])) {
                    return ['mess' => '<div align="center">Lỗi! Không có sản phẩm nào được chọn</div>'];
                }
                if (empty($reqData['priceNew']) && empty($reqData['quantityNew'])) {
                    return ['mess' => '<div align="center">Lỗi! Bạn vui lòng nhập số lượng hoặc đơn giá mới</div>'];
                }
                //create arr id
                $arrId = explode(',', $reqData['arrId']);
                if (!is_array($arrId)) {
                    return ['mess' => '<div align="center">Lỗi! Danh sách khách hàng không đúng định dạng</div>'];
                }
                //get arr customer
                $orderDetails = TbOrdersDetail::findAll($arrId);
                if (empty($orderDetails))
                    return ['mess' => '<div align="center">Lỗi! Không tìm thấy danh sách khách hàng</div>'];

                $orderID = empty($orderDetails[0]['orderID']) ? 0 : $orderDetails[0]['orderID'];
                $order = TbOrders::findOne(['orderID' => $orderID]);
                if (empty($orderDetails) || empty($order))
                    return ['mess' => '<div align="center">Lỗi! Không tìm thấy đơn hàng</div>'];

                foreach ($orderDetails as $key => $orderDetail) {
                    if (!empty($reqData['priceNew']))
                        $orderDetail->unitPrice = $reqData['priceNew'];
                    if (!empty($reqData['quantityNew']))
                        $orderDetail->quantity = $reqData['quantityNew'];
                    $orderDetail->totalPrice = $orderDetail->quantity * $orderDetail->unitPrice; //tien tq
                    $orderDetail->unitPriceVn = round($orderDetail->unitPrice * $order->cny, 1); //tien viet
                    $orderDetail->totalPriceVn = $orderDetail->quantity * $orderDetail->unitPriceVn;
                    $orderDetail->reasonPrice = $reqData['reasonPrice'];
                    $orderDetail->update(false);

                    $tbHistory = new TbHistory();
                    $tbHistory->orderID = $order->orderID;
                    $tbHistory->userID = Yii::$app->user->id;
                    $tbHistory->content = Yii::$app->user->identity->username . '</b><br/>Đã chỉnh sửa sản phẩm : <b>' . $orderDetail->productID . '</b><br/>';
                    $tbHistory->content .= '';
                    $tbHistory->content .= '<br/>Đơn hàng: <b><a target="_blank" title="Chi tiết đơn hàng" href="' . Url::toRoute(['orders/view', 'id' => $order->orderID]) . '"><i class="fa fa-eye" aria-hidden="true"></i>' . $order->identify . '</a></b>';
                    $tbHistory->save(false);
                }

                $customer = $order->customer;
                $cnys = isset($customer->cny) ? $customer->cny : '';
                $cny = CommonLib::getCNY($this->setting['CNY'], $cnys, $order->cny);
                ...
                if (CommonLib::updateOrder($order)) {
                    return ['mess' => '<div align="center"><span class="label label-success"><i class="fa fa-check-circle"></i></span> Cập nhật số lượng, giá thành công!</div>'];
                }
                return ['mess' => '<div align="center">Lỗi! Cập nhật số lượng, giá không thành công.</div>'];
            }
            return ['mess' => '<div align="center">Lỗi! Phương thức không đúng</div>'];
        }
        ...
        /*
         * tu choi thanh toanh or tu choi chuyen trang thai sang da dat hang
         * */
        public function actionPayreject()
        {
            \Yii::$app->response->format = Response::FORMAT_JSON;
            ...
            if (\Yii::$app->request->isAjax) {
                ...
                if (!empty($orderIds) || $orderID) {
                    ...
                    //tim don hang trang thai = 2 va da gui yeu cau thanh toan
                    if ($orders = TbOrders::find()->where(['orderID' => $orderIds, 'status' => 2, 'requestPay' => 1])->all()) {
                        TbOrders::updateAll(['requestPay' => 0], ['orderID' => $orderIds, 'status' => 2]);
                        ...
                        foreach ($orders as $order) {
                            ...
                            $tbHistory->createDate = date('Y-m-d H:i:s');
                            $tbHistory->content = 'Quản trị: <b>' . Yii::$app->user->identity->username . '</b><br/>';
                            $tbHistory->content .= 'Từ chối thanh toánh đơn hàng ' . $order->identify . '</b><br/>';
                            $tbHistory->save(false);
                        }
            ...
            if (\Yii::$app->request->isAjax) {
                ...
                if (!empty($orderIds) || $orderID) {
                    if (!empty($orderIds))
                        ...
                        $orderIds = $orderID;
                    //chi update cho don hang trang thai = 2
                    if ($orders = TbOrders::find()->where(['and', ['status' => 2, 'orderID' => $orderIds], ['!=', 'requestPay', 1]])->asArray()->all()) {
                        TbOrders::updateAll(['requestPay' => 1, 'updatetime' => date('Y-m-d H:i:s')], ['and', ['status' => 2, 'orderID' => $orderIds], ['!=', 'requestPay', 1]]);
                        ...
                        $tbHistory->createDate = date('Y-m-d H:i:s');
                        $tbHistory->content = 'Quản trị: <b>' . Yii::$app->user->identity->username . '</b><br/>';
                        $tbHistory->content .= 'Gửi yêu cầu đặt hàng các đơn hàng: ' . implode(',', array_column($orders, 'identify')) . '</b>';
                        $tbHistory->save(false);

                        $alert = 'Cập nhật thành công';
                        $error = 0;
                    } else {
                        $alert = 'Cập nhật thất bại. Đơn hàng đã được chuyển chờ thanh toán.';
                    }
