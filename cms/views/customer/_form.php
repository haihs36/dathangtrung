...
<div class="box">
    ...
    <div class="box-body">
        ...
        <div class="tab-content box-body">
            ...
            ])->textInput(['maxlength' => true, 'placeholder' => '% Cọc'])->label('% Cọc', ['class' => "col-sm-2 control-label"]) ?>

            <?= $form->field($model, 'discountRate', [
                'template' => '{label}(<i>%</i>)<div class="col-sm-2">{input}{error}</div>'
            ])->textInput(['maxlength' => true, 'placeholder' => '% phí dịch vụ'])->label('% phí dịch vụ', ['class' => "col-sm-2 control-label"]) ?>

            <?= $form->field($model, 'discountKg', [
                'template' => '{label}(<i class="vnd-unit">VNĐ</i>)<div class="col-sm-2">{input}{error}</div>'
            ])->textInput(['class' => 'currency form-control', 'placeholder' => 'Phí cân nặng'])->label('Phí cân nặng', ['class' => "col-sm-2 control-label"]) ?>

            <?= $form->field($model, 'weightFee', [
                'template' => '{label}(<i class="vnd-unit">VNĐ</i>)<div class="col-sm-2">{input}{error}</div>'
            ])->textInput(['class' => 'currency form-control', 'placeholder' => 'Phí cân nặng ký gửi'])->label('Phí cân nặng ký gửi', ['class' => "col-sm-2 control-label"]) ?>
                <?= $form->field($model, 'cny', [
                    'template' => '{label}(<i class="vnd-unit">VNĐ</i>)<div class="col-sm-2">{input}{error}</div>'
                ])->textInput(['class' => 'currency form-control', 'placeholder' => 'Tỷ giá'])->label('Tỷ giá', ['class' => "col-sm-2 control-label"]) ?>
