...
$this->params['breadcrumbs'][] = ['label' => 'Quản lý bắn mã vạch', 'url' => ['barcode']];
...
<script>
    $(function () {
        ...
        $('.modal-container').html(<?= trim($alert) ?>);
                        ...
                        'delete' => function($url, $model) {
                            if ($model->status != 1 || Yii::$app->user->identity->role === ADMIN) {
                                return ' <div class="btn-group btn-group-sm" role="group">
                                        <a href="' . \yii\helpers\Url::to(['shipping/delete-code', 'id' => $model->id]) . '" class="confirm-delete" title="MVĐ: ' . $model->shippingCode . '"><span class="glyphicon glyphicon-remove"></span> Delete</a>
                                     </div>';
                            }
                            return null;
                        }
